...
| `callbackRegistry.register('keep_order', (data, deps) => …)` | `bot_callback:keep_order` with no `handles` edge; unresolved `registry-handler-anonymous` |
...
| `bot.action('back_to_main', …)` in the same function | `bot_callback:back_to_main` handled by `registerMenuHandlers` itself, `meta.handlerVia` `registration` |
