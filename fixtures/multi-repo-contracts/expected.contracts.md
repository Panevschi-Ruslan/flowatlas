...
contracts: edges=17 shared=2 identical=2 drift=13 unchecked=5 errors=13 warnings=3 infos=4 ignored=4
...
| info | extra_field | gateway → orders | request | debugId | sender gateway sends `debugId: string`; receiver orders declares no such field |
...
| info | extra_field | gateway → orders | request | internalNote | sender gateway sends `internalNote: string`; receiver orders declares no such field; the receiver excludes it, so what arrives is thrown away |
...
| info | extra_field | gateway → orders | request | code | sender gateway sends `code: string`; receiver orders declares no such field |
...
| no-type-on-sender | 1 | Pass a typed value rather than a literal, so there is a declared shape to compare. |
