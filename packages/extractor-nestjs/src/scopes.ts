 ...
 * channel (R54). Two readers with the same sentence to say is one sentence, and
 * it lives here — in the package both adapter packages already depend on for
 * the context, the class index and `methodBodies` — rather than in either of
 * them, because a copy per adapter would diverge the first time one of them
 * reconsidered what counts as a body.
 *
 * It belongs above the adapters and below no framework: `@flowatlas/core` is
 * where the pieces come from, but the answer is in terms of a
 * `NestExtractContext` — which functions the entries pass has already named,
 * which classes the index kept — and that context is this package's.
 */
import {
  memberFunction,
  methodBodies,
...
export const repoSourceFiles = function* (ctx: NestExtractContext): Generator<SourceFile> {
...
const scopeOf = (ctx: NestExtractContext, fn: NamedFunction): Scope => ({
...
const classMethods = function* (ctx: NestExtractContext, seen: Seen): Generator<Scope> {
 ...
 * name and a node.
 */
const registeredHandlers = function* (ctx: NestExtractContext, seen: Seen): Generator<Scope> {
 ...
 * result reads like.
 */
export const scopesOf = function* (ctx: NestExtractContext): Generator<Scope> {
